...
function switchMainTab(tabName) {
    ...
    // Load prompt files list when switching to admin tab
    if (tabName === 'admin') {
        loadPromptFilesList();
    }
}
...
// Load prompt files list on admin tab switch
async function loadPromptFilesList() {
    try {
        const response = await fetchWithTimeout('http://localhost:8050/api/prompts/list');
        const data = await response.json();

        if (data.success && data.files) {
            const select = document.getElementById('promptFileSelect');
            ...
            alert('Failed to load prompt files: ' + (data.message || 'Unknown error'));
        }
    } catch (error) {
        console.error('Error loading prompt files:', error);
        alert('Error loading prompt files: ' + error.message);
    }
}

// Load a prompt file and its versions
async function loadPromptFile() {
    const filename = document.getElementById('promptFileSelect').value;
    if (!filename) {
        document.getElementById('promptVersionSelect').innerHTML = '<option value="current">Current (Active)</option>';
        return;
    }

    promptEditorState.currentFile = filename;
    promptEditorState.currentVersion = 'current';

    // Load versions list
    try {
        const response = await fetchWithTimeout(`http://localhost:8050/api/prompts/versions/list/${filename}`);
        const data = await response.json();

        const versionSelect = document.getElementById('promptVersionSelect');
        versionSelect.innerHTML = '<option value="current">Current (Active)</option>';
        ...
        if (data.success && data.versions && data.versions.length > 0) {
            data.versions.forEach(version => {
                ...
                option.textContent = `${version.name} (${date})`;
...
async function loadPromptVersion() {
    ...
    const versionName = document.getElementById('promptVersionSelect').value;
    ...
    promptEditorState.currentVersion = versionName;
}
...
async function openPromptEditor() {
    ...
    const versionName = promptEditorState.currentVersion;

    try {
        let response;
        if (versionName === 'current') {
            response = await fetchWithTimeout(`http://localhost:8050/api/prompts/content/${filename}`);
        } else {
            response = await fetchWithTimeout(`http://localhost:8050/api/prompts/versions/content/${filename}/${versionName}`);
        }
        ...
        if (data.success) {
            ...
            document.getElementById('promptVersionLabel').textContent = versionName === 'current' ? 'Current (Active)' : versionName;
            document.getElementById('promptContent').value = data.content;
            document.getElementById('promptEditorModal').style.display = 'flex';
            promptEditorState.hasUnsavedChanges = false;

            // Add change listener
            document.getElementById('promptContent').addEventListener('input', () => {
                promptEditorState.hasUnsavedChanges = true;
            });
        } else {
            alert('Failed to load prompt content: ' + (data.message || 'Unknown error'));
        }
    } catch (error) {
        console.error('Error opening prompt editor:', error);
        alert('Error opening prompt editor: ' + error.message);
    }
}

// Close the prompt editor
function closePromptEditor() {
    if (promptEditorState.hasUnsavedChanges) {
...
async function savePromptChanges() {
    ...
    if (promptEditorState.currentVersion !== 'current') {
        alert('You can only save changes to the current (active) version. Please activate this version first or select "Current (Active)" to edit.');
        return;
    }

    try {
        const response = await fetchWithTimeout('http://localhost:8050/api/prompts/update', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.success) {
            ...
            alert('Failed to update prompt: ' + (data.message || 'Unknown error'));
        }
    } catch (error) {
...
async function createPromptVersion() {
    ...
    const versionName = prompt('Enter a name for this version (or leave blank for timestamp):');
    if (versionName === null) return; // User cancelled

    try {
        const response = await fetchWithTimeout('http://localhost:8050/api/prompts/versions/create', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                filename, 
        ...
        if (data.success) {
            alert(`Version created: ${data.version_name}`);
            await loadPromptFile(); // Reload versions list
        } else {
            alert('Failed to create version: ' + (data.message || 'Unknown error'));
        }
    } catch (error) {
...
async function activatePromptVersion() {
    ...
    if (!confirm(`Are you sure you want to activate version "${versionName}"? The current active version will be backed up automatically.`)) {
        return;
    }

    try {
        const response = await fetchWithTimeout('http://localhost:8050/api/prompts/versions/activate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ filename, version_name: versionName })
        });

        const data = await response.json();

        if (data.success) {
            alert(`Version "${versionName}" is now active!`);
            document.getElementById('promptVersionSelect').value = 'current';
            promptEditorState.currentVersion = 'current';
            await loadPromptFile(); // Reload versions list
        } else {
            alert('Failed to activate version: ' + (data.message || 'Unknown error'));
        }
    } catch (error) {
...
async function deletePromptVersion() {
    ...
    if (versionName === 'current') {
        alert('Cannot delete the current active version. Select a different version to delete.');
        return;
    }
    ...
    try {
        const response = await fetchWithTimeout('http://localhost:8050/api/prompts/versions/delete', {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ filename, version_name: versionName })
        });
        ...
        if (data.success) {
            alert(`Version "${versionName}" deleted successfully`);
            document.getElementById('promptVersionSelect').value = 'current';
            promptEditorState.currentVersion = 'current';
            await loadPromptFile(); // Reload versions list
        } else {
            alert('Failed to delete version: ' + (data.message || 'Unknown error'));
        }
    } catch (error) {
...
/**
 * Open the help editor modal and load current help content
 */
async function openHelpEditor() {
    try {
        ...
        document.getElementById('helpEditorModal').style.display = 'flex';
    } catch (error) {
...
/**
 * Save changes to help content
 */
async function saveHelpChanges() {
    const content = document.getElementById('helpContent').value;
    
    if (!content.trim()) {
        alert('Help content cannot be empty');
        return;
    }
    ...
    try {
        ...
        const data = await response.json();
        if (data.success) {
            alert('Help documentation updated successfully');
            closeHelpEditor();
        } else {
            alert('Failed to update help: ' + (data.message || 'Unknown error'));
        }
    } catch (error) {
